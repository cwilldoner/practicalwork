...
$`\textbf{x}_i`$ is a feature map, consisting of $`n_i`$ feature maps, each of size $`h_i`$ and $`w_i`$. E.g. for the two consecutive layers ```self.conv1 = Conv2d(1,6,(5,5))``` and ```self.conv2 = Conv2d(6,16,(5,5))```, $`n_1=1`$, $`n_2=6`$ and $`n_3=16`$. A kernel matrix $`F_i`$ consists of $`n_{i+1}`$ kernel filters e.g. ```6x(5,5)```, whereas a kernel filter $`F_{i,j}`$ consists of $`n_i`$ kernels (e.g. ```1x(5,5)```) of size $`\Kappa \in \mathbb{R}^{k x k}`$. The number of operations for a whole layer without pruning is $`n_{i+1}*(n_i*k*k*h_{i+1}*w_{i+1})`$. When a filter is removed, the number of operations is reduced by $`(n_i*k*k*h_{i+1}*w_{i+1})`$, since a whole feature map is removed. The now removed feature map leads to an removed filter in the next layer which leads to an additional reduction of $`(n_{i+2}*k*k*h_{i+2}*w_{i+2})`$ operations. With n hyperparameter $m$ one can define how many filters should be removed from the kernel matrix (number of blue columns in the above figure).
