...
def initialize_weights_fixup(module):
    if isinstance(module, BasicBlock):
        ...
        b = module
        if isinstance(b.conv1, nn.Sequential):
            n = b.conv1[0].kernel_size[0] * b.conv1[0].kernel_size[1] * b.conv1[0].out_channels
            b.conv1[0].weight.data.normal_(0, (layer_index_total ** (-0.5)) * math.sqrt(2. / n))
            b.conv1[3].weight.data.normal_(0, (layer_index_total ** (-0.5)) * math.sqrt(2. / n))
            #b.conv1[3].weight.data.zero_()
            if isinstance(b.conv2, nn.Sequential):
                n = b.conv2[0].kernel_size[0] * b.conv2[0].kernel_size[1] * b.conv2[0].out_channels
                b.conv2[0].weight.data.normal_(0, (layer_index_total ** (-0.5)) * math.sqrt(2. / n))
                b.conv2[3].weight.data.normal_(0, (layer_index_total ** (-0.5)) * math.sqrt(2. / n))
            else:
...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, out_channels, k1=3, k2=3, groups=1):
        ...
        if k1 == 3 and self.layer_index > 0:
...
class Network(nn.Module):
    def __init__(self, config  # ff_weight_anticolapse_limit=0.5
        ...
        #     MBConv(n_channels[0], n_channels[0]),
            ...
            for m_i, mp_pos in enumerate(maxpool):
                if index + 1 == mp_pos:
                    stage.add_module("maxpool{}_{}".format(index + 1, m_i),
                                     nn.MaxPool2d((2, 1), stride=(2, 1)))
        #stage.add_module("Dropout", nn.Dropout(p=0.2, inplace=False))
        return stage
...
#     def __init__(self, in_features: int, out_features: int, expansion: int = 6, stride: int = 1):
...
#                         # wide -> narrow
#                         ConvNormAct(expanded_features, out_features, kernel_size=1, act=nn.Identity),
#                     ),
#                     shortcut=ConvNormAct(in_features, out_features, kernel_size=1, stride=stride, act=nn.Identity)
